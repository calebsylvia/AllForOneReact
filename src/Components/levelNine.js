import { levelNineCall } from "../Services/Data-Service";
...
export default function LevelNine() {
    ...
    const getResult = async () => {
      const response = await levelNineCall(input);
      setResult(response);
    }
  return (
    ...
    <div className="pl-3">
      <div className="font-text text-neutral-100 max-[769px]:text-center">
        <p className="text-8xl max-[321px]:text-6xl">LEVEL 9</p>
        <p className="text-lg sm:text-3xl">RESTAURANT PICKER!</p>
      </div>
      ...
      <div className="font-text mt-12 lg:mt-52 w-72 md:w-96 mx-auto lg:mx-0">
        <p className="text-neutral-100 text-nowrap text-3xl max-[769px]:hidden">
          PICK ASIAN, MEXICAN, OR INDIAN:{" "}
        </p>
        <input
          id="input"
          type="text"
          className="rounded-md w-72 md:w-96 h-10 sm:h-12 pl-3 text-xl"
          placeholder="ASIAN,MEXICAN,INDIAN..."
